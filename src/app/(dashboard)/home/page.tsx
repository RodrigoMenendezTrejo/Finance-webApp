...
import { FinancialOverviewCard, RecentActivityCard, GoalsCard, BudgetsCard } from '@/components/bento/quick-stats-card';
...
import { getBudgetSummary, getBudgetProgress } from '@/lib/firebase/budget-service';
...
export default function DashboardPage() {
    ...
    // Budget summary data
    const [budgetSummary, setBudgetSummary] = useState({ count: 0, usedPercent: 0, overBudgetCount: 0 });
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Fetch budget summary for the budgets card
            const budgetSum = await getBudgetSummary(user.uid);
            const usedPct = budgetSum.totalBudgeted > 0
                ? (budgetSum.totalSpent / budgetSum.totalBudgeted) * 100
                : 0;
            setBudgetSummary({
                count: budgetSum.budgetCount,
                usedPercent: usedPct,
                overBudgetCount: budgetSum.overBudgetCount,
            });

            // Show budget notifications if any thresholds exceeded (only once per session)
            const notificationsShown = sessionStorage.getItem('budgetNotificationsShown');
            if (!notificationsShown) {
                const budgetProgress = await getBudgetProgress(user.uid);
                let hasAlerts = false;
                budgetProgress.forEach((bp) => {
                    const catName = getCategoryById(bp.budget.categoryId)?.name || bp.budget.categoryId;
                    if (bp.isOverBudget) {
                        hasAlerts = true;
                        toast.error(`${catName}: Over Budget!`, {
                            description: `Spent ${formatCurrency(bp.spent)} of ${formatCurrency(bp.budget.amount)} (${bp.percentUsed.toFixed(0)}%)`,
                            duration: 6000,
                        });
                    } else if (bp.isNearLimit) {
                        hasAlerts = true;
                        toast.warning(`${catName}: Near Limit`, {
                            description: `${bp.percentUsed.toFixed(0)}% of budget used (${formatCurrency(bp.remaining)} remaining)`,
                            duration: 5000,
                        });
                    }
                });
                if (hasAlerts) {
                    sessionStorage.setItem('budgetNotificationsShown', 'true');
                }
            }
    ...
    return (
        <main className="min-h-screen pb-24">
            ...
            <BentoGrid>
                ...
                {/* Budget Limits */}
                <BudgetsCard
                    budgetCount={budgetSummary.count}
                    usedPercent={budgetSummary.usedPercent}
